...
const templateHits = `
        ...
        <div class="col s12 m6">
          <span class="text-bold">{{#helpers.highlight}}{ "attribute": "Description" }{{/helpers.highlight}}</span>
        </div>
          <div class="col s12 m5">
          <a class="truncate text-light" title="Data web-link">{{ Numer }}</a>
          </div>
          <div class="col m1 hide-on-small-only">
          <div class="actions-wrapper center-align">
              <a class="text-bold">
                <span class="text-bold">
                {{#helpers.highlight}}{ "attribute": "Pages" }{{/helpers.highlight}}
                </span>
              </a>
          </div>
        </div>
      ...
      <span class="small text-light">
          <p> Department: {{#Dept}}
          {{#helpers.highlight}}{ "attribute": "Dept" }{{/helpers.highlight}}
          {{/Dept}} </p>
      </span>
      </div>
      <div class="col s12 m5">
...
const DeptList = instantsearch.widgets.panel({
  templates: {
    header: '<i class="fa fa-chevron-right"></i> Dept',
  },
  cssClasses: {
    header: 'panel-header',
  },
...
search.addWidget(
  DeptList({
        container: `#ais-widget-refinement-list--Dept`,
        attribute: 'Dept',
        limit: 8,
        showMore: true,
        showMoreLimit: 20,
